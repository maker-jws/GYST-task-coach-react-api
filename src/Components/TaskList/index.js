...
class TaskList extends Component {
  ...
  render() {
    ...
    const displayTaskList = this.props.taskList.map(task => {
      // console.log(this.props.currentUser.user_id, "<-- user in da state"); gets user ID
      // console.log(task.user_id); this gets user id from each task
      if (this.props.currentUser.user_id === task.user_id) {
        taskCount++;
